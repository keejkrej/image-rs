...
use self::interaction::roi::RoiKind;
use self::interaction::tooling::{
    LineMode, OvalMode, PointMode, RectMode, ToolId, ToolOptionsState, ToolState,
};
use self::interaction::transform::{ViewerTransformState, zoom_level_down, zoom_level_up};
use ndarray::IxDyn;
use rfd::FileDialog;
...
impl ImageUiApp {
    fn new(cc: &eframe::CreationContext<'_>, startup_input: Option<PathBuf>) -> Self {
        let menus: Vec<MenuManifestTopLevel> =
            serde_json::from_str(include_str!("ui/menu/imagej-menu-manifest.json"))
...
fn load_toolbar_icons(ctx: &egui::Context) -> HashMap<ToolbarIcon, egui::TextureHandle> {
    let specs = [
        (
            ToolbarIcon::Rect,
            "rect",
            include_bytes!("ui/assets/tools/rect.png").as_slice(),
        ),
        (
            ToolbarIcon::Oval,
            "oval",
            include_bytes!("ui/assets/tools/oval.png").as_slice(),
        ),
        (
            ToolbarIcon::Poly,
            "poly",
            include_bytes!("ui/assets/tools/poly.png").as_slice(),
        ),
        (
            ToolbarIcon::Free,
            "free",
            include_bytes!("ui/assets/tools/free.png").as_slice(),
        ),
        (
            ToolbarIcon::Line,
            "line",
            include_bytes!("ui/assets/tools/line.png").as_slice(),
        ),
        (
            ToolbarIcon::Angle,
            "angle",
        ...
        (
            ToolbarIcon::Point,
            "point",
            include_bytes!("ui/assets/tools/point.png").as_slice(),
        ),
        (
            ToolbarIcon::Wand,
            "wand",
            include_bytes!("ui/assets/tools/wand.png").as_slice(),
        ),
        (
            ToolbarIcon::Text,
            "text",
            include_bytes!("ui/assets/tools/text.png").as_slice(),
        ),
        (
            ToolbarIcon::Open,
            "open",
            include_bytes!("ui/assets/tools/open.png").as_slice(),
        ),
        (
            ToolbarIcon::Zoom,
            "zoom",
            include_bytes!("ui/assets/tools/zoom.png").as_slice(),
        ),
        (
            ToolbarIcon::Hand,
            "hand",
            include_bytes!("ui/assets/tools/hand.png").as_slice(),
        ),
        (
            ToolbarIcon::Dropper,
            "dropper",
            include_bytes!("ui/assets/tools/dropper.png").as_slice(),
        ),
        (
            ToolbarIcon::Previous,
            "previous",
            include_bytes!("ui/assets/tools/previous.png").as_slice(),
        ),
        (
            ToolbarIcon::Next,
            "next",
            include_bytes!("ui/assets/tools/next.png").as_slice(),
        ),
        (
            ToolbarIcon::Quit,
            "quit",
            include_bytes!("ui/assets/tools/quit.png").as_slice(),
        ),
        (
            ToolbarIcon::Custom1,
            "custom1",
            include_bytes!("ui/assets/tools/custom1.png").as_slice(),
        ),
        (
            ToolbarIcon::Custom2,
            "custom2",
            include_bytes!("ui/assets/tools/custom2.png").as_slice(),
        ),
        (
            ToolbarIcon::Custom3,
            "custom3",
            include_bytes!("ui/assets/tools/custom3.png").as_slice(),
        ),
        (
            ToolbarIcon::More,
            "more",
            include_bytes!("ui/assets/tools/more.png").as_slice(),
        ),
    ];
