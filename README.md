...
- Safe plugin package discovery with strict manifests, SemVer compatibility, namespaced contributions, and a versioned WebAssembly Component contract; a host/library registration API adapts compatible scoped operations through a bounded no-WASI Wasmtime sandbox, while automatic CLI/UI discovery remains future work; see [`docs/plugin-system.md`](docs/plugin-system.md)
- A shared `OpsService::describe`/`invoke` seam distinguishes one-shot whole-dataset operations from active-plane, active-Z-stack, and deterministic all-plane 2D processing. Native and Wasm adapters accept exact area masks, stage output atomically, and expose caller cancellation plus structured plane/guest progress.
...
- Parameterized plane operations default to the active C/Z/T plane; eligible stacks add a `Process stack` choice that holds C/T and iterates Z. Rectangle, oval, polygon, and freehand selections become exact processing masks. Work runs off the GPUI thread, Escape/close/quit can cancel it, and a result is committed only if its viewer dataset revision is still current; a pixel replacement creates exactly one undo entry.
- Analyze > Measure samples only the active C/Z/T plane and exact ROI, with calibrated ImageJ-style columns controlled by application-wide Set Measurements settings; Measure Stack appends one row per Z plane at the active C/T. Stored results retain full precision while the Results window and CSV export honor the selected decimal places.
